...
```
[![Verified by Asqav](data:image/svg+xml,...)](https://www.asqav.com/verify/<record_id>)
```

The badge image is an inline SVG generated by the badge page, so use the snippet the generator produces rather than hand-writing a URL. Each `signature_id` gets its own verifier link.
