...
- Internal helpers `_compute_tool_fingerprint(tool_name, tool_schema)` and `_generate_nonce()` for callers that want to precompute either field.
- Parametrised tests over the five new fields plus rule-9 happy / sad paths (`python/tests/test_client_nsa_aligned_fields.py`).
...
- Public helpers `computeToolFingerprint(toolName, toolSchema)` and `generateNonce()` re-exported from `@asqav/sdk` for callers that want to precompute either field.
- New test file `typescript/tests/nsaAlignedFields.test.ts` mirroring the Python parametrised cases for parity.
